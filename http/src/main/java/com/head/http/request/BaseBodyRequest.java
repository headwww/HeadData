...
import com.head.http.body.ProgressResponseCallBack;
import com.head.http.body.RequestBodyUtils;
import com.head.http.body.UploadProgressRequestBody;
    ...
    extends BaseRequest<R>
{
    ...
    public R params(String key, InputStream stream, String fileName,
        ProgressResponseCallBack responseCallBack)
    {
    ...
    public R params(String key, byte[] bytes, String fileName,
        ProgressResponseCallBack responseCallBack)
    {
    ...
    public <T> R params(String key, T file, String fileName,
        MediaType contentType, ProgressResponseCallBack responseCallBack)
    {
    ...
    protected Observable<ResponseBody> uploadFilesWithParts()
    {
        ...
        // 拼接参数键值对
        for (Map.Entry<String, String> mapEntry : params.urlParamsMap
            .entrySet())
        {
            parts.add(MultipartBody.Part.createFormData(mapEntry.getKey(),
                mapEntry.getValue()));
        }
        // 拼接文件
        for (Map.Entry<String, List<HttpParams.FileWrapper>> entry : params.fileParamsMap
            .entrySet())
        {
    ...
    protected Observable<ResponseBody> uploadFilesWithBodys()
    {
            ...
            .entrySet())
        {
            ...
            for (HttpParams.FileWrapper fileWrapper : fileValues)
            {
                ...
                UploadProgressRequestBody uploadProgressRequestBody =
                    new UploadProgressRequestBody(requestBody,
    ...
    // 文件方式
    private MultipartBody.Part addFile(String key,
        HttpParams.FileWrapper fileWrapper)
    {
        ...
        if (fileWrapper.responseCallBack != null)
        {
            UploadProgressRequestBody uploadProgressRequestBody =
                new UploadProgressRequestBody(requestBody,
                    fileWrapper.responseCallBack);
            MultipartBody.Part part = MultipartBody.Part.createFormData(key,
        ...
        else
        {
            MultipartBody.Part part = MultipartBody.Part
                .createFormData(key, fileWrapper.fileName, requestBody);
            return part;
        }
    }
    ...
    private RequestBody getRequestBody(HttpParams.FileWrapper fileWrapper)
    {
        ...
        {
            requestBody = RequestBody.create(fileWrapper.contentType,
                (File)fileWrapper.file);
        }
        else if (fileWrapper.file instanceof InputStream)
        {
            // requestBody =
            // RequestBodyUtils.create(RequestBodyUtils.MEDIA_TYPE_MARKDOWN,
            // (InputStream) fileWrapper.file);
            requestBody = RequestBodyUtils.create(fileWrapper.contentType,
                (InputStream)fileWrapper.file);
        }
        else if (fileWrapper.file instanceof byte[])
        {
            requestBody = RequestBody.create(fileWrapper.contentType,
                (byte[])fileWrapper.file);
        }
        return requestBody;
    }
}

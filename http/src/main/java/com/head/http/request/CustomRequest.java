...
public class CustomRequest extends BaseRequest<CustomRequest>
{
    ...
    /**
     * 调用call返回一个Observable<T> 举例：如果你给的是一个Observable<ApiResult<AuthModel>>
     * 那么返回的<T>是一个ApiResult<AuthModel>
     */
    ...
    {
        observable.compose(RxUtil.io_main()).subscribe(subscriber);
    }
    ...
    public <T> Disposable apiCall(Observable<T> observable,
        CallBack<T> callBack)
    {
    ...
    public <T> Disposable call(Observable<T> observable,
        CallBackProxy<? extends ApiResult<T>, T> proxy)
    {
        Observable<CacheResult<T>> cacheobservable =
            build().toObservable(observable, proxy);

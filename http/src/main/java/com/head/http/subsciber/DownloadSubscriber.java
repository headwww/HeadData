...
import com.head.http.callback.DownloadProgressCallBack;
    ...
    extends BaseSubscriber<ResponseBody>
{
    ...
    private static String APK_CONTENTTYPE =
    ...
    private boolean writeResponseBodyToDisk(String path, String name,
        Context context, okhttp3.ResponseBody body)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    HttpLog.i("file download: " + fileSizeDownloaded + " of "
                        + fileSize);
                    // 下载进度
                    ...
                    if (curTime - lastRefreshUiTime >= 200 || progress == 1.0f)
                    {
                        if (callBack != null)
                        {
                            if (callBack != null)
                            {
                                final long finalFileSizeDownloaded =
                                    fileSizeDownloaded;
                ...
                outputStream.flush();
                HttpLog.i("file downloaded: " + fileSizeDownloaded + " of "
                    + fileSize);

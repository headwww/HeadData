...
import com.head.http.cache.core.CacheCore;
import com.head.http.cache.core.LruDiskCache;
import com.head.http.cache.model.CacheMode;
import com.head.http.cache.model.CacheResult;
import com.head.http.cache.stategy.IStrategy;
 ...
 * .diskDir(new File(getCacheDir().getPath() + File.separator +
 * "data-cache"))//不设置，默认使用缓存路径<br>
...
public final class RxCache
{
    ...
    private RxCache(Builder builder)
    {
        ...
        cacheCore = new CacheCore(
            new LruDiskCache(diskConverter, diskDir, appVersion, diskMaxSize));
    }
    ...
    public <T> ObservableTransformer<T, CacheResult<T>> transformer(
        CacheMode cacheMode, final Type type)
    {
        final IStrategy strategy = loadStrategy(cacheMode);// 获取缓存策略
        return new ObservableTransformer<T, CacheResult<T>>()
        {
            @Override
            public ObservableSource<CacheResult<T>> apply(
                @NonNull Observable<T> upstream)
            {
                ...
                return strategy.execute(RxCache.this,
                    RxCache.this.cacheKey,
                    RxCache.this.cacheTime,
                    upstream,
                    tempType);
            }
        };
    ...
    private IStrategy loadStrategy(CacheMode cacheMode)
    {
        try
        {
            ...
            return (IStrategy)Class
                .forName(pkName + "." + cacheMode.getClassName())
                .newInstance();
        }
        catch (Exception e)
        {
            throw new RuntimeException(
                "loadStrategy(" + cacheMode + ") err!!" + e.getMessage());
        }
    }
    ...
    public static final class Builder
    {
        ...
        private static long calculateDiskCacheSize(File dir)
        {
            ...
            try
            {
                ...
                long available =
                    ((long)statFs.getBlockCount()) * statFs.getBlockSize();
                size = available / 50;
            }
        ...
        public File getDiskCacheDir(Context context, String uniqueName)
        {
            ...
            if (cacheDir == null)
            {// if cacheDir is null throws NullPointerException
                cacheDir = context.getCacheDir();
            }

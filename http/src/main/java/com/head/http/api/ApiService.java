...
/**
 *
 * 类名称：ApiService.java <br/>
 * 类描述：通用的的api接口<br/>
 * 创建人：shuwen <br/>
 * 创建时间：2019-10-24 09:15 <br/>
 * 
 * @version
 */
public interface ApiService
{
    ...
    Observable<ResponseBody> postJson(@Url String url,
    ...
    // @DELETE()//delete body请求比较特殊 需要自定义
    @HTTP(method = "DELETE", /* path = "", */hasBody = true)
    Observable<ResponseBody> deleteBody(@Url String url,
                                        @Body RequestBody body);
    
    // @DELETE()//delete body请求比较特殊 需要自定义
    @Headers({"Content-Type: application/json", "Accept: application/json"})
    @HTTP(method = "DELETE", /* path = "", */hasBody = true)
    Observable<ResponseBody> deleteJson(@Url String url,
                                        @Body RequestBody jsonBody);
    ...
    @PUT()
    @Headers({"Content-Type: application/json", "Accept: application/json"})
    Observable<ResponseBody> putJson(@Url String url,
                                     @Body RequestBody jsonBody);

 ...
 * max-age=3600) @GET("merchants/{shopId}/icon") Observable<ShopIconEntity>
 * getShopIcon(@Path("shopId") long shopId);
 * 
...
public class CacheInterceptorOffline extends CacheInterceptor
{
        ...
        throws IOException
    {
        ...
        {
            HttpLog.i(
                " no network load cache:" + request.cacheControl().toString());
            /*
             * request = request.newBuilder() .removeHeader("Pragma")
             * .header("Cache-Control", "only-if-cached, " +
             * cacheControlValue_Offline) .build();
             */
            ...
            return response.newBuilder()
                ...
                .header("Cache-Control",
                    "public, only-if-cached, " + cacheControlValue_Offline)
                .build();
        }

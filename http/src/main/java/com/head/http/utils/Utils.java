...
public class Utils
{
    ...
    public static RequestBody createFile(String name)
    {
        ...
        return RequestBody.create(
            okhttp3.MediaType.parse("multipart/form-data; charset=utf-8"),
            name);
    }
    ...
    public static RequestBody createImage(File file)
    {
        checkNotNull(file, "file not null!");
        return RequestBody
            .create(okhttp3.MediaType.parse("image/jpg; charset=utf-8"), file);
    }
    ...
    public static void closeThrowException(Closeable close)
        throws IOException
    {
        if (close != null)
        {
    ...
    public static <T> List<Type> getMethodTypes(Class<T> cls)
    {
        ...
        if (typeOri instanceof ParameterizedType)
        {
            ...
            Type[] parentypes =
                ((ParameterizedType)typeOri).getActualTypeArguments();
            for (Type childtype : parentypes)
            {
                ...
                {
                    Type[] childtypes =
                        ((ParameterizedType)childtype).getActualTypeArguments();
    ...
    public static Type getParameterizedType(Type type, int i)
    {
        ...
        { // 处理泛型类型
            Type genericType =
                ((ParameterizedType)type).getActualTypeArguments()[i];
            return genericType;
        }
        else if (type instanceof TypeVariable)
        {

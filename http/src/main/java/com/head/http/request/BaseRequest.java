...
import com.head.http.api.ApiService;
...
import com.head.http.interceptor.BaseDynamicInterceptor;
import com.head.http.interceptor.CacheInterceptor;
import com.head.http.interceptor.CacheInterceptorOffline;
import com.head.http.interceptor.HeadersInterceptor;
import com.head.http.interceptor.NoCacheInterceptor;
...
public abstract class BaseRequest<R extends BaseRequest>
{
    ...
    {
        this.url = url;
        context = HeadHttp.getContext();
        HeadHttp config = HeadHttp.getInstance();
            ...
            && (url.startsWith("http://") || url.startsWith("https://")))
        {
            ...
            baseUrl = httpUrl.url().getProtocol() + "://"
                + httpUrl.url().getHost() + "/";
        }
    ...
    {
        if (cacheTime <= -1)
            cacheTime = HeadHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    {
        interceptors
            .add(Utils.checkNotNull(interceptor, "interceptor == null"));
        return (R)this;
    }
    ...
    public R addCookie(String name, String value)
    {
        ...
        Cookie cookie =
            builder.name(name).value(value).domain(httpUrl.host()).build();
     ...
     * https双向认证证书
     */
    public R certificates(InputStream bksFile, String password,
    ...
    private OkHttpClient.Builder generateOkClient()
    {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0
            && sslParams == null && cookies.size() == 0
            && hostnameVerifier == null && proxy == null && headers.isEmpty())
        {
            OkHttpClient.Builder builder = HeadHttp.getOkHttpClientBuilder();
        ...
        else
        {
            final OkHttpClient.Builder newClientBuilder =
                HeadHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
                newClientBuilder.readTimeout(readTimeOut,
                    TimeUnit.MILLISECONDS);
            if (writeTimeOut > 0)
                newClientBuilder.writeTimeout(writeTimeOut,
                    TimeUnit.MILLISECONDS);
            if (connectTimeout > 0)
                newClientBuilder.connectTimeout(connectTimeout,
                    TimeUnit.MILLISECONDS);
            if (hostnameVerifier != null)
                newClientBuilder.hostnameVerifier(hostnameVerifier);
            if (sslParams != null)
                newClientBuilder.sslSocketFactory(sslParams.sSLSocketFactory,
                    sslParams.trustManager);
            if (proxy != null)
                newClientBuilder.proxy(proxy);
            if (cookies.size() > 0)
                HeadHttp.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit()
    {
        ...
        {
            Retrofit.Builder builder = HeadHttp.getRetrofitBuilder();
        ...
        else
        {
            ...
            {
                // 获取全局的对象重新设置
                Retrofit.Builder newBuilder = HeadHttp.getRetrofitBuilder();
                if (!TextUtils.isEmpty(baseUrl))
                {
                    newBuilder.baseUrl(baseUrl);
                }
                List<Converter.Factory> listConverterFactory =
                    newBuilder.build().converterFactories();
    ...
    /**
     * 根据当前的请求参数，生成对应的RxCache和Cache缓存机制
     */
    private RxCache.Builder generateRxCache()
    {
        final RxCache.Builder rxCacheBuilder = HeadHttp.getRxCacheBuilder();
        switch (cacheMode)
        {
            case NO_CACHE:// 不使用缓存
            ...
            case DEFAULT:// 使用Okhttp的缓存
                if (this.cache == null)
                {
                    File cacheDirectory = HeadHttp.getCacheDirectory();
                    if (cacheDirectory == null)
                    {
                        cacheDirectory =
                            new File(HeadHttp.getContext().getCacheDir(),
                                "okhttp-cache");
                    }
                    else
                    {
                    ...
                    this.cache = new Cache(cacheDirectory,
                        Math.max(5 * 1024 * 1024, HeadHttp.getCacheMaxSize()));
                }
                String cacheControlValue =
                    String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR =
                    new CacheInterceptor(HeadHttp.getContext(),
                        cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE =
                    new CacheInterceptorOffline(HeadHttp.getContext(),
                        cacheControlValue);
                networkInterceptors.add(REWRITE_CACHE_CONTROL_INTERCEPTOR);
                networkInterceptors
                    .add(REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE);
            ...
            case CACHEANDREMOTEDISTINCT:
                ...
                {
                    final RxCache.Builder cacheBuilder =
                        HeadHttp.getRxCache().newBuilder();

...
import com.head.http.cache.converter.IDiskConverter;
import com.head.http.cache.converter.SerializableDiskConverter;
import com.head.http.cache.model.CacheMode;
import com.head.http.cookie.CookieManger;
import com.head.http.https.HttpsUtils;
import com.head.http.interceptor.HttpLoggingInterceptor;
import com.head.http.model.HttpHeaders;
import com.head.http.model.HttpParams;
import com.head.http.request.CustomRequest;
import com.head.http.request.DeleteRequest;
import com.head.http.request.DownloadRequest;
import com.head.http.request.GetRequest;
import com.head.http.request.PostRequest;
import com.head.http.request.PutRequest;
...
/**
 *
 * 类名称：HeadHttp.java <br/>
 * 网络请求入口类
 * </p>
 * 主要功能：</br>
 * 1.全局设置超时时间 2.支持请求错误重试相关参数，包括重试次数、重试延时时间</br>
...
public final class HeadHttp
{
    ...
    private volatile static HeadHttp singleton = null;
    
    private HeadHttp()
    {
        okHttpClientBuilder = new OkHttpClient.Builder();
        okHttpClientBuilder.hostnameVerifier(new DefaultHostnameVerifier());
        okHttpClientBuilder.connectTimeout(DEFAULT_MILLISECONDS,
            TimeUnit.MILLISECONDS);
        okHttpClientBuilder.readTimeout(DEFAULT_MILLISECONDS,
            TimeUnit.MILLISECONDS);
        okHttpClientBuilder.writeTimeout(DEFAULT_MILLISECONDS,
            TimeUnit.MILLISECONDS);
    ...
    public static HeadHttp getInstance()
    {
        testInitialize();
        if (singleton == null)
        {
            synchronized (HeadHttp.class)
            {
                if (singleton == null)
                {
                    singleton = new HeadHttp();
                }
            }
        }
        return singleton;
    ...
    private static void testInitialize()
    {
        if (sContext == null)
            throw new ExceptionInInitializerError(
                "请先在全局Application中调用 HeadHttp.init() 初始化！");
    }
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public HeadHttp debug(String tag)
    {
        debug(tag, true);
        return this;
    }
     ...
     */
    public HeadHttp debug(String tag, boolean isPrintException)
    {
        String tempTag = TextUtils.isEmpty(tag) ? "RxHeadHttp_" : tag;
        if (isPrintException)
        {
            HttpLoggingInterceptor loggingInterceptor =
                new HttpLoggingInterceptor(tempTag, isPrintException);
     ...
     * https的全局访问规则
     */
    public HeadHttp setHostnameVerifier(HostnameVerifier hostnameVerifier)
    {
     ...
     * https的全局自签名证书
     */
    public HeadHttp setCertificates(InputStream... certificates)
    {
     ...
     * https双向认证证书
     */
    public HeadHttp setCertificates(InputStream bksFile, String password,
     ...
     * 全局cookie存取规则
     */
    public HeadHttp setCookieStore(CookieManger cookieManager)
    {
     ...
     * 全局读取超时时间
     */
    public HeadHttp setReadTimeOut(long readTimeOut)
    {
     ...
     * 全局写入超时时间
     */
    public HeadHttp setWriteTimeOut(long writeTimeout)
    {
     ...
     * 全局连接超时时间
     */
    public HeadHttp setConnectTimeout(long connectTimeout)
    {
        okHttpClientBuilder.connectTimeout(connectTimeout,
            TimeUnit.MILLISECONDS);
        return this;
    }
     ...
     * 超时重试次数
     */
    public HeadHttp setRetryCount(int retryCount)
    {
        if (retryCount < 0)
     ...
     * 超时重试延迟时间
     */
    public HeadHttp setRetryDelay(int retryDelay)
    {
        if (retryDelay < 0)
     ...
     * 超时重试延迟叠加时间
     */
    public HeadHttp setRetryIncreaseDelay(int retryIncreaseDelay)
    {
     ...
     * 全局的缓存模式
     */
    public HeadHttp setCacheMode(CacheMode cacheMode)
    {
     ...
     * 全局的缓存过期时间
     */
    public HeadHttp setCacheTime(long cacheTime)
    {
        if (cacheTime <= -1)
     ...
     * 全局的缓存大小,默认50M
     */
    public HeadHttp setCacheMaxSize(long maxSize)
    {
     ...
     */
    public HeadHttp setCacheVersion(int cacheersion)
    {
        if (cacheersion < 0)
     ...
     */
    public HeadHttp setCacheDirectory(File directory)
    {
     ...
     * 全局设置缓存的转换器
     */
    public HeadHttp setCacheDiskConverter(IDiskConverter converter)
    {
        rxCacheBuilder
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public HeadHttp setHttpCache(Cache cache)
    {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public HeadHttp addCommonParams(HttpParams commonParams)
    {
     ...
     * 添加全局公共请求参数
     */
    public HeadHttp addCommonHeaders(HttpHeaders commonHeaders)
    {
     ...
     * 添加全局拦截器
     */
    public HeadHttp addInterceptor(Interceptor interceptor)
    {
     ...
     * 添加全局网络拦截器
     */
    public HeadHttp addNetworkInterceptor(Interceptor interceptor)
    {
     ...
     * 全局设置代理
     */
    public HeadHttp setOkproxy(Proxy proxy)
    {
     ...
     * 全局设置请求的连接池
     */
    public HeadHttp setOkconnectionPool(ConnectionPool connectionPool)
    {
        okHttpClientBuilder.connectionPool(
            Utils.checkNotNull(connectionPool, "connectionPool == null"));
        return this;
    }
     ...
     */
    public HeadHttp setOkclient(OkHttpClient client)
    {
     ...
     */
    public HeadHttp addConverterFactory(Converter.Factory factory)
    {
     ...
     */
    public HeadHttp addCallAdapterFactory(CallAdapter.Factory factory)
    {
     ...
     */
    public HeadHttp setCallbackExecutor(Executor executor)
    {
        retrofitBuilder
            .callbackExecutor(Utils.checkNotNull(executor, "executor == null"));
        return this;
    }
     ...
     */
    public HeadHttp setCallFactory(okhttp3.Call.Factory factory)
    {
        retrofitBuilder
            .callFactory(Utils.checkNotNull(factory, "factory == null"));
        return this;
    }
     ...
     * 全局设置baseurl
     */
    public HeadHttp setBaseUrl(String baseUrl)
    {
